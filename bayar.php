  ...
  <main id="main">
    ...
    <div class="wrapper">
      <div class="list-group">
        ...
        <a href="e_password.php" class="list-group-item list-group-item-action">Ganti Kata Sandi</a>
      ...
      <div class="core">
        <div class="col">
          <div class="card">
            ...
            <div class="card-body">
              <p>Silakan Transfer Uang ke No Rekenning Di Bawah Ini Sebesar :</p>
              <?php
              $id_bukti = $_GET['id_bukti'];
              $query = mysqli_query($koneksi, "SELECT id_transaksi, total FROM tb_transaksi WHERE id_user = '$id_user' AND id_bukti = '$id_bukti'");
              $data = mysqli_fetch_row($query);
              $id_transaksi = $data[0];
              $hargaTotalKeranjang = (int)$data[1];
              ?>
              <h3 class="my-3" style="color: #ff6d00;">Rp, <?= number_format($hargaTotalKeranjang); ?></h3>
              <table class="table">
                <thead>

                  <tr>
                    <th scope="col">Bank</th>
                    <th scope="col">No Rekenning</th>
        ...
        <div class="col mt-4">
          <div class="card">
            ...
            <form method="POST" action="bukti_upload.php" enctype="multipart/form-data">
              <input type="text" name="id_transaksi" value="<?= $id_transaksi; ?>" hidden>
              <div class="card-body">
                <div class="mb-3">
                  <label class="form-label" style="font-weight: bold;">Bukti Transfer</label>
        ...
        <?php
        $id_bukti = $_GET['id_bukti'];
        $query = mysqli_query($koneksi, "SELECT * FROM tb_transaksi WHERE id_bukti = '$id_bukti'");
        $data = mysqli_fetch_row($query);
        $in = $data[1];
        ?>
        <a href="d_transaksi.php?in=<?= $in; ?>" class="btn btn-md btn-warning my-2">Kembali</a>
      </div>
    </div>
    <section></section>
  </main><!-- End #main -->
